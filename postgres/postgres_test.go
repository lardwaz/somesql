...
func TestQuery_AsSQL_ConditionClause(t *testing.T) {
	...
	tests := []testCase{
		...
		{
			name:           "WHERE id=?",
			query:          New().Select("data").Where(somesql.And(somesql.LangEN, "id", "=", "1")),
			expectedSQL:    "SELECT data_en FROM repo WHERE id=?",
			expectedValues: []interface{}{"1"},
		},
		{
			name:           "WHERE id=? AND status=?",
			query:          New().Select("data").Where(somesql.And(somesql.LangEN, "id", "=", "1")).Where(somesql.And(somesql.LangEN, "status", "=", "published")),
			expectedSQL:    "SELECT data_en FROM repo WHERE id=? AND status=?",
			expectedValues: []interface{}{"1", "published"},
		},
		{
			name:           "WHERE id=? OR status=?",
			query:          New().Select("data").Where(somesql.And(somesql.LangEN, "id", "=", "1")).Where(somesql.Or(somesql.LangEN, "status", "=", "published")),
			expectedSQL:    "SELECT data_en FROM repo WHERE id=? OR status=?",
			expectedValues: []interface{}{"1", "published"},
		},
		{
			name:           "WHERE id=? AND status=? OR type=?",
			query:          New().Select("data").Where(somesql.And(somesql.LangEN, "id", "=", "1")).Where(somesql.And(somesql.LangEN, "status", "=", "published")).Where(somesql.Or(somesql.LangEN, "type", "=", "article")),
		...
		// Regular condition clauses on json attributes
		{
			...
			query:          New().Select("data").Where(somesql.And(somesql.LangEN, "author_id", "=", "1")),
			expectedSQL:    "SELECT data_en FROM repo WHERE data_en->>'author_id'=?",
			expectedValues: []interface{}{"1"},
		},
		{
			name:           "WHERE data_fr->>'author_id'=? (langFR)",
			query:          New().Select("data").SetLang(somesql.LangFR).Where(somesql.And(somesql.LangEN, "author_id", "=", "1")),
			expectedSQL:    "SELECT data_fr FROM repo WHERE data_fr->>'author_id'=?",
			expectedValues: []interface{}{"1"},
		},
		{
			name:           "WHERE data_fr->>'author_id'=? OR data_fr->>'category_id'=? (langFR)",
			query:          New().Select("data").SetLang(somesql.LangFR).Where(somesql.And(somesql.LangEN, "author_id", "=", "1")).Where(somesql.Or(somesql.LangEN, "category_id", "=", "2")),
		...
		},
		{
			...
			query:          New().Select("data").SetLang(somesql.LangFR).Where(somesql.And(somesql.LangEN, "author_id", "=", "1")).Where(somesql.And(somesql.LangEN, "category_id", "=", "2")),
...
func TestQuery_AsSQL_ConditionGroup(t *testing.T) {
	...
	tests := []testCase{
		{
			name:           "WHERE (... OR ...) [1]",
			query:          New().Select("data").Where(somesql.AndGroup(somesql.And(somesql.LangEN, "badge", "=", "video"), somesql.Or(somesql.LangEN, "badge", "=", "audio"))),
			expectedSQL:    `SELECT data_en FROM repo WHERE ("data_en"->>'badge'=? OR ("data_en"->>'badge')=?)`,
			expectedValues: []interface{}{"video", "audio"},
		},
		{
			name:           "WHERE (... OR ...) [2]",
			query:          New().Select("data").Where(somesql.OrGroup(somesql.And(somesql.LangEN, "badge", "=", "video"), somesql.Or(somesql.LangEN, "badge", "=", "audio"))),
			expectedSQL:    `SELECT data_en FROM repo WHERE ("data_en"->>'badge'=? OR ("data_en"->>'badge')=?)`,
			expectedValues: []interface{}{"video", "audio"},
		},
		{
			name:           "WHERE (... AND ...) [1]",
			query:          New().Select("data").Where(somesql.AndGroup(somesql.Or(somesql.LangEN, "badge", "=", "video"), somesql.And(somesql.LangEN, "has_video", "=", true))),
		...
		{
			name:           "WHERE (... AND ...) [2]",
			query:          New().Select("data").Where(somesql.OrGroup(somesql.Or(somesql.LangEN, "badge", "=", "video"), somesql.And(somesql.LangEN, "has_video", "=", true))),
			expectedSQL:    `SELECT data_en FROM repo WHERE ("data_en"->>'badge'=? AND ("data_en"->>'has_video')::BOOLEAN=?)`,
			expectedValues: []interface{}{"published", "video", true},
		},
		{
			name:           "WHERE (... AND ...) AND (... OR ...) [1]",
			query:          New().Select("data").Where(somesql.OrGroup(somesql.Or(somesql.LangEN, "badge", "=", "video"), somesql.And(somesql.LangEN, "has_video", "=", true))).Where(somesql.AndGroup(somesql.Or(somesql.LangEN, "badge", "=", "video"), somesql.Or(somesql.LangEN, "has_video", "=", true))),
		...
		},
		{
			...
			query:          New().Select("data").Where(somesql.OrGroup(somesql.Or(somesql.LangEN, "badge", "=", "video"), somesql.And(somesql.LangEN, "has_video", "=", true))).Where(somesql.OrGroup(somesql.Or(somesql.LangEN, "badge", "=", "video"), somesql.And(somesql.LangEN, "has_video", "=", true))),
	...
	tests := []testCase{
		{
			...
			query:          New().Select("data").Where(somesql.AndIn(somesql.LangEN, "id", []string{"A", "B", "C"})),
			expectedSQL:    "SELECT data_en FROM repo WHERE id IN (?,?,?)",
			expectedValues: []interface{}{"A", "B", "C"},
		},
		{
			name:           "WHERE id IN (...) - primitive field - LangFR",
			query:          New().Select("data").Where(somesql.AndIn(somesql.LangEN, "id", []string{"A", "B", "C"})).SetLang(somesql.LangFR),
			expectedSQL:    "SELECT data_fr FROM repo WHERE id IN (?,?,?)",
			expectedValues: []interface{}{"A", "B", "C"},
		},
		{
			name:           "WHERE field IN (...) - JSONB",
			query:          New().Where(somesql.AndIn(somesql.LangEN, "name", []string{"A", "B", "C"})),
		...
		},
		{
			...
			query:          New().Where(somesql.AndIn(somesql.LangEN, "name", []string{"A", "B", "C"})).SetLang(somesql.LangFR),
		...
		},
		{
			...
			query:          New().Select("data").Where(somesql.AndIn(somesql.LangEN, "updated_at", []string{"2019"}, "YEAR")),
			expectedSQL:    "SELECT data_en FROM repo WHERE YEAR(updated_at) IN (?)",
			expectedValues: []interface{}{"2019"},
		},
		{
			name:           "WHERE FUNC(field) IN (...) - JSONB",
			query:          New().Where(somesql.AndIn(somesql.LangEN, "name", []string{"a"}, "LOWER")),
...

		{
			...
			query:          New().Select("data").Where(somesql.AndNotIn(somesql.LangEN, "id", []string{"A", "B", "C"})),
			expectedSQL:    "SELECT data_en FROM repo WHERE id NOT IN (?,?,?)",
			expectedValues: []interface{}{"A", "B", "C"},
		},
		{
			name:           "WHERE id NOT IN (...) - primitive field - LangFR",
			query:          New().Select("data").Where(somesql.AndNotIn(somesql.LangEN, "id", []string{"A", "B", "C"})).SetLang(somesql.LangFR),
			expectedSQL:    "SELECT data_fr FROM repo WHERE id NOT IN (?,?,?)",
			expectedValues: []interface{}{"A", "B", "C"},
		},
		{
			name:           "WHERE FUNC(field) NOT IN (...) - primitive field",
			query:          New().Select("data").Where(somesql.AndNotIn(somesql.LangEN, "updated_at", []string{"2019"}, "YEAR")),
			expectedSQL:    "SELECT data_en FROM repo WHERE YEAR(updated_at) NOT IN (?)",
			expectedValues: []interface{}{"2019"},
		},
		{
			name:           "WHERE field NOT IN (...) - JSONB",
			query:          New().Where(somesql.AndNotIn(somesql.LangEN, "name", []string{"A", "B", "C"})),
...

		{
			...
			query:          New().Select("data").Where(somesql.AndIn(somesql.LangEN, "id", []string{"A", "B"})).Where(somesql.AndNotIn(somesql.LangEN, "id", []string{"C", "D"})),
		...
		},
		{
			...
			query:          New().Select("data").Where(somesql.AndIn(somesql.LangEN, "name", []string{"A", "B"})).Where(somesql.AndNotIn(somesql.LangEN, "id", []string{"C", "D"})),
			expectedSQL:    "SELECT data_en FROM repo WHERE id IN (?,?) AND id NOT IN (?,?)",
			expectedValues: []interface{}{"A", "B", "C", "D"},
		},
		{
			name:           "WHERE id IN (...) OR NOT IN (...) - primitive field",
			query:          New().Select("data").Where(somesql.AndIn(somesql.LangEN, "id", []string{"A", "B"})).Where(somesql.OrNotIn(somesql.LangEN, "id", []string{"C", "D"})),
			expectedSQL:    "SELECT data_en FROM repo WHERE id IN (?,?) OR id NOT IN (?,?)",
			expectedValues: []interface{}{"A", "B", "C", "D"},
		},
		{
			name:           "WHERE FUNC(field) IN (...) AND field NOT IN (...) - primitive field",
			query:          New().Select("data").Where(somesql.AndIn(somesql.LangEN, "updated_at", []string{"2019"}, "YEAR")).Where(somesql.AndNotIn(somesql.LangEN, "id", []string{"A", "B"})),
...

		{
			...
			query:          New().Select("data").Where(somesql.AndIn(somesql.LangEN, "type", []string{"article", "dossier"})).Where(somesql.And(somesql.LangEN, "status", "=", []string{"published"})),
		...
		{
			name:           "WHERE FUNC(field) IN (...) AND field NOT IN (...) - primitive field",
			query:          New().Select("data").Where(somesql.AndIn(somesql.LangEN, "updated_at", []string{"2019"}, "YEAR")).Where(somesql.AndNotIn(somesql.LangEN, "id", []string{"A", "B"})).Where(somesql.And(somesql.LangEN, "status", "=", "published")),
		...
		},
		{
			...
			query:          New().Select("data").Where(somesql.AndIn(somesql.LangEN, "tag_ids", []string{"A"})).Where(somesql.AndIn(somesql.LangEN, "author_ids", []string{"B"})).Where(somesql.And(somesql.LangEN, "status", "=", "published")),
		...
		{
			name:           "WHERE id IN (...) AND field = ...",
			query:          New().Select("data").Where(somesql.AndGroup(somesql.And(somesql.LangEN, "badge", "=", "video"), somesql.And(somesql.LangEN, "has_video", "=", true))).Where(somesql.And(somesql.LangEN, "status", "=", []string{"published"})),
		...
		},
		{
			...
			query:          New().Select("data").Where(somesql.And(somesql.LangEN, "status", "=", []string{"published"})).Where(somesql.OrGroup(somesql.And(somesql.LangEN, "badge", "=", "video"), somesql.And(somesql.LangEN, "has_video", "=", true))),
		...
		},
		{
			...
			query:          New().Select("data").Where(somesql.Or(somesql.LangEN, "status", "=", []string{"published"})).Where(somesql.OrGroup(somesql.Or(somesql.LangEN, "badge", "=", "video"), somesql.And(somesql.LangEN, "has_video", "=", true))),
			expectedSQL:    `SELECT data_en FROM repo WHERE status =? OR ("data_en"->>'badge'=? AND ("data_en"->>'has_video')::BOOLEAN=?)`,
			expectedValues: []interface{}{"published", "video", true},
		},
		{
			name:           "WHERE field = ... AND (... OR ...) [1]",
			query:          New().Select("data").Where(somesql.Or(somesql.LangEN, "status", "=", []string{"published"})).Where(somesql.AndGroup(somesql.And(somesql.LangEN, "badge", "=", "video"), somesql.Or(somesql.LangEN, "has_video", "=", true))),
		...
		},
		{
			...
			query:          New().Select("data").Where(somesql.Or(somesql.LangEN, "status", "=", []string{"published"})).Where(somesql.AndGroup(somesql.And(somesql.LangEN, "badge", "=", "video"), somesql.Or(somesql.LangEN, "badge", "=", "audio"))),
...
func TestQuesy_AsSQL_InQuery(t *testing.T) {
	...
	tests := []testCase{
		{
			name:           "AndInQuery",
			query:          New().Select("data").Where(somesql.AndIn(somesql.LangEN, "id", []string{"A", "B", "C"})).Where(somesql.AndInQuery("author_id", New().Select("author_id").Where(somesql.And(somesql.LangEN, "id", "=", "002fd6b1-f715-4875-838b-1546f27327df")))),
			expectedSQL:    `SELECT data_en FROM repo WHERE id IN (?,?,?) AND "data_en"->>'author_id' IN (SELECT json_build_object('author_id', data_en->'author_id') "data" FROM repo WHERE id =?)`,
			expectedValues: []interface{}{"A", "B", "C", "002fd6b1-f715-4875-838b-1546f27327df"},
		},
		{
			name:           "AndNotInQuery",
			query:          New().Select("data").Where(somesql.AndIn(somesql.LangEN, "id", []string{"A", "B", "C"})).Where(somesql.AndNotInQuery("author_id", New().Select("author_id").Where(somesql.And(somesql.LangEN, "id", "=", "002fd6b1-f715-4875-838b-1546f27327df")))),
			expectedSQL:    `SELECT data_en FROM repo WHERE id IN (?,?,?) AND "data_en"->>'author_id' NOT IN (SELECT json_build_object('author_id', data_en->'author_id') "data" FROM repo WHERE id =?)`,
			expectedValues: []interface{}{"A", "B", "C", "002fd6b1-f715-4875-838b-1546f27327df"},
		},

		{
			name:           "OrInQuery",
			query:          New().Select("data").Where(somesql.AndIn(somesql.LangEN, "id", []string{"A", "B", "C"})).Where(somesql.OrInQuery("author_id", New().Select("author_id").Where(somesql.And(somesql.LangEN, "id", "=", "002fd6b1-f715-4875-838b-1546f27327df")))),
			expectedSQL:    `SELECT data_en FROM repo WHERE id IN (?,?,?) OR "data_en"->>'author_id' IN (SELECT json_build_object('author_id', data_en->'author_id') "data" FROM repo WHERE id =?)`,
			expectedValues: []interface{}{"A", "B", "C", "002fd6b1-f715-4875-838b-1546f27327df"},
		},
		{
			name:           "OrNotInQuery",
			query:          New().Select("data").Where(somesql.AndIn(somesql.LangEN, "id", []string{"A", "B", "C"})).Where(somesql.OrNotInQuery("author_id", New().Select("author_id").Where(somesql.And(somesql.LangEN, "id", "=", "002fd6b1-f715-4875-838b-1546f27327df")))),
			expectedSQL:    `SELECT data_en FROM repo WHERE id IN (?,?,?) OR "data_en"->>'author_id' NOT IN (SELECT json_build_object('author_id', data_en->'author_id') "data" FROM repo WHERE id =?)`,
			expectedValues: []interface{}{"A", "B", "C", "002fd6b1-f715-4875-838b-1546f27327df"},
		},
	}
