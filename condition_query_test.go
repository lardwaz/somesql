...
func TestConditionQuery(t *testing.T) {
	...
	tests := []testCase{
		// Error: cannot have more than 1 field in subquery
		{
			...
			fieldName: "author_id",
			query:     somesql.NewSelectInner().Fields("type", "data.slug").Where(somesql.And(somesql.LangEN, "id", "=", "002fd6b1-f715-4875-838b-1546f27327df")),
		...
		},
		{
			name:      "AndInQuery [1]",
			fieldName: "author_id",
			query:     somesql.NewSelectInner().Fields("data.author_id").Where(somesql.And(somesql.LangEN, "id", "=", "1")),
