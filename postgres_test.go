...
func TestQuery_AsSQL_Insert(t *testing.T) {
	...
	tests := []testCase{
		// Insert
		{
			...
			query:          somesql.NewInsert().Fields(somesql.NewFields().UseDefaults().ID("1").CreatedAt(time.Date(2009, time.November, 10, 23, 0, 0, 0, time.UTC)).UpdatedAt(time.Date(2009, time.November, 10, 23, 0, 0, 0, time.UTC)).Status("published")),
		...
		},
		{
			...
			query:          somesql.NewInsert().Fields(somesql.NewFields().ID("1").Status("published")),
			expectedSQL:    `INSERT INTO repo ("id", "status") VALUES ($1, $2)`,
			expectedValues: []interface{}{"1", "published"},
		},
		{
			name:           "INSERT no default + 1 relation",
			query:          somesql.NewInsert().Fields(somesql.NewFields().Status("published").Set("relations.tags", []string{"a", "b", "c"})),
		...
		},
		{
			...
			query:          somesql.NewInsert().Fields(somesql.NewFields().Status("published").Set("relations.author", []string{"x"}).Set("relations.tags", []string{"a", "b", "c"})),
			expectedSQL:    `INSERT INTO repo ("status", "relations") VALUES ($1, $2)`,
...
func TestQuery_AsSQL_Update(t *testing.T) {
	...
	tests := []testCase{
		// Update
		{
			...
			query:          somesql.NewUpdate().Fields(somesql.NewFields().ID("1").Status("published")),
		...
		},
		{
			...
			expectedSQL:    `UPDATE repo SET "data_en" = COALESCE("data_en" || json_object(ARRAY['body', $1, 'author_id', $2])::JSONB, json_object(ARRAY['body', $3, 'author_id', $4])::JSONB)`,
			expectedValues: []interface{}{"body value", "123", "body value", "123"},
		},
		{
			...
			expectedSQL:    `UPDATE repo SET "data_fr" = COALESCE("data_fr" || json_object(ARRAY['body', $1, 'author_id', $2])::JSONB, json_object(ARRAY['body', $3, 'author_id', $4])::JSONB)`,
			expectedValues: []interface{}{"body value", "123", "body value", "123"},
		},
		{
			name:           "UPDATE meta + data fields",
			query:          somesql.NewUpdate().Fields(somesql.NewFields().ID("1").Status("published").Set("data.body", "body value").Set("data.author_id", "123")),
			expectedSQL:    `UPDATE repo SET "id" = $1, "status" = $2, "data_en" = COALESCE("data_en" || json_object(ARRAY['body', $3, 'author_id', $4])::JSONB, json_object(ARRAY['body', $5, 'author_id', $6])::JSONB)`,
			expectedValues: []interface{}{"1", "published", "body value", "123", "body value", "123"},
		},
		{
			name:           "UPDATE meta + data fields (LangFR)",
			query:          somesql.NewUpdateLang(somesql.LangFR).Fields(somesql.NewFields().ID("1").Status("published").Set("data.body", "body value").Set("data.author_id", "123")),
			expectedSQL:    `UPDATE repo SET "id" = $1, "status" = $2, "data_fr" = COALESCE("data_fr" || json_object(ARRAY['body', $3, 'author_id', $4])::JSONB, json_object(ARRAY['body', $5, 'author_id', $6])::JSONB)`,
			expectedValues: []interface{}{"1", "published", "body value", "123", "body value", "123"},
		},
		{
			...
			expectedSQL:    `UPDATE repo SET "id" = $1, "status" = $2, "data_en" = COALESCE("data_en" || json_object(ARRAY['body', $3, 'author_id', $4])::JSONB, json_object(ARRAY['body', $5, 'author_id', $6])::JSONB) WHERE "id" = $7`,
			expectedValues: []interface{}{"1", "published", "body value", "123", "body value", "123", "234"},
		},
		{
			...
			expectedSQL:    `UPDATE repo SET "id" = $1, "status" = $2, "data_en" = COALESCE("data_en" || json_object(ARRAY['body', $3, 'author_id', $4])::JSONB, json_object(ARRAY['body', $5, 'author_id', $6])::JSONB) WHERE "data_en"->>'author_id' = $7`,
			expectedValues: []interface{}{"1", "published", "body value", "123", "body value", "123", "234"},
		},
