...
func TestQuery_AsSQL_ConditionClause(t *testing.T) {
	...
	tests := []testCase{
		// Regular condition clauses on pre-defined fields
		{
			...
			expectedSQL:    "SELECT data_en FROM repo WHERE id=? LIMIT 10",
		...
		},
		{
			...
			expectedSQL:    "SELECT data_en FROM repo WHERE id=? AND status=? LIMIT 10",
		...
		},
		{
			...
			expectedSQL:    "SELECT data_en FROM repo WHERE id=? OR status=? LIMIT 10",
			expectedValues: []interface{}{"1", "published"},
		},
		{
			...
			expectedSQL:    "SELECT data_en FROM repo WHERE id=? AND status=? OR type=? LIMIT 10",
		...
		// Regular condition clauses on json attributes
		{
			...
			expectedSQL:    `SELECT data_en FROM repo WHERE "data_en"->>'author_id'=? LIMIT 10`,
			expectedValues: []interface{}{"1"},
		},
		{
			...
			expectedSQL:    `SELECT data_fr FROM repo WHERE "data_fr"->>'author_id'=? LIMIT 10`,
			expectedValues: []interface{}{"1"},
		},
		{
			...
			expectedSQL:    `SELECT data_fr FROM repo WHERE "data_fr"->>'author_id'=? OR "data_fr"->>'category_id'=? LIMIT 10`,
		...
		},
		{
			...
			expectedSQL:    `SELECT data_fr FROM repo WHERE "data_fr"->>'author_id'=? AND "data_fr"->>'category_id'=? LIMIT 10`,
			expectedValues: []interface{}{"1", "2"},
		},
	}
	...
	tests := []testCase{
		{
			...
			expectedSQL:    `SELECT data_en FROM repo WHERE ("data_en"->>'badge'=? OR "data_en"->>'badge'=?) LIMIT 10`,
		...
		},
		{
			...
			expectedSQL:    `SELECT data_en FROM repo WHERE ("data_en"->>'badge'=? AND ("data_en"->>'has_video')::BOOLEAN=?) LIMIT 10`,
		...
		},
		{
			...
			expectedSQL:    `SELECT data_en FROM repo WHERE ("data_en"->>'badge'=? AND ("data_en"->>'has_video')::BOOLEAN=?) AND ("data_en"->>'badge'=? OR ("data_en"->>'has_video')::BOOLEAN=?) LIMIT 10`,
		...
		},
		{
			...
			expectedSQL:    `SELECT data_en FROM repo WHERE ("data_en"->>'badge'=? AND ("data_en"->>'has_video')::BOOLEAN=?) OR ("data_en"->>'badge'=? AND ("data_en"->>'has_video')::BOOLEAN=?) LIMIT 10`,
			expectedValues: []interface{}{"video", true, "video", true},
		},
	}
	...
	tests := []testCase{
		{
			...
			expectedSQL:    "SELECT data_en FROM repo WHERE id IN (?,?,?) LIMIT 10",
		...
		},
		{
			...
			expectedSQL:    "SELECT data_fr FROM repo WHERE id IN (?,?,?) LIMIT 10",
			expectedValues: []interface{}{"A", "B", "C"},
		},
		{
			name:           "WHERE field IN (...) - JSONB",
			query:          somesql.NewQuery().Where(somesql.AndIn(somesql.LangEN, "name", []string{"A", "B", "C"})),
			expectedSQL:    `SELECT id, created_at, updated_at, owner_id, status, type, data_en FROM repo WHERE "data_en"->>'name' IN (?,?,?) LIMIT 10`,
			expectedValues: []interface{}{"A", "B", "C"},
		},
		{
			...
			expectedSQL:    `SELECT id, created_at, updated_at, owner_id, status, type, data_fr FROM repo WHERE "data_fr"->>'name' IN (?,?,?) LIMIT 10`,
			expectedValues: []interface{}{"A", "B", "C"},
		},
		{
			...
			expectedSQL:    "SELECT data_en FROM repo WHERE YEAR(updated_at) IN (?) LIMIT 10",
		...
		},
		{
			...
			expectedSQL:    `SELECT id, created_at, updated_at, owner_id, status, type, data_en FROM repo WHERE LOWER("data_en"->>'name') IN (?) LIMIT 10`,
...

		{
			...
			expectedSQL:    "SELECT data_en FROM repo WHERE id NOT IN (?,?,?) LIMIT 10",
			expectedValues: []interface{}{"A", "B", "C"},
		},
		{
			...
			expectedSQL:    "SELECT data_fr FROM repo WHERE id NOT IN (?,?,?) LIMIT 10",
			expectedValues: []interface{}{"A", "B", "C"},
		},
		{
			...
			expectedSQL:    "SELECT data_en FROM repo WHERE YEAR(updated_at) NOT IN (?) LIMIT 10",
			expectedValues: []interface{}{"2019"},
		},
		{
			name:           "WHERE field NOT IN (...) - JSONB",
			query:          somesql.NewQuery().Where(somesql.AndNotIn(somesql.LangEN, "name", []string{"A", "B", "C"})),
			expectedSQL:    `SELECT id, created_at, updated_at, owner_id, status, type, data_en FROM repo WHERE "data_en"->>'name' NOT IN (?,?,?) LIMIT 10`,
			expectedValues: []interface{}{"A", "B", "C"},
		},
		{
			name:           "WHERE field NOT IN (...) - JSONB",
		...
		},
		{
			...
			expectedSQL:    `SELECT id, created_at, updated_at, owner_id, status, type, data_en FROM repo WHERE LOWER("data_en"->>'name') NOT IN (?) LIMIT 10`,
			expectedValues: []interface{}{"a"},
		},

		{
			...
			expectedSQL:    "SELECT data_en FROM repo WHERE id IN (?,?) AND id NOT IN (?,?) LIMIT 10",
		...
		},
		{
			...
			expectedSQL:    `SELECT data_en FROM repo WHERE "data_en"->>'name' IN (?,?) AND id NOT IN (?,?) LIMIT 10`,
			expectedValues: []interface{}{"A", "B", "C", "D"},
		},
		{
			...
			expectedSQL:    "SELECT data_en FROM repo WHERE id IN (?,?) OR id NOT IN (?,?) LIMIT 10",
			expectedValues: []interface{}{"A", "B", "C", "D"},
		},
		{
			...
			expectedSQL:    "SELECT data_en FROM repo WHERE YEAR(updated_at) IN (?) AND id NOT IN (?,?) LIMIT 10",
...

		{
			...
			expectedSQL:    "SELECT data_en FROM repo WHERE type IN (?,?) AND status=? LIMIT 10",
		...
		},
		{
			...
			expectedSQL:    "SELECT data_en FROM repo WHERE YEAR(updated_at) IN (?) AND id NOT IN (?,?) AND status=? LIMIT 10",
		...
		},
		{
			...
			expectedSQL:    `SELECT data_en FROM repo WHERE "data_en"->>'tag_ids' IN (?) AND "data_en"->>'author_ids' IN (?) AND status=? LIMIT 10`,
...

		{
			...
			expectedSQL:    `SELECT data_en FROM repo WHERE ("data_en"->>'badge'=? AND ("data_en"->>'has_video')::BOOLEAN=?) AND status=? LIMIT 10`,
		...
		},
		{
			...
			expectedSQL:    `SELECT data_en FROM repo WHERE status=? OR ("data_en"->>'badge'=? AND ("data_en"->>'has_video')::BOOLEAN=?) LIMIT 10`,
		...
		},
		{
			...
			expectedSQL:    `SELECT data_en FROM repo WHERE status=? AND ("data_en"->>'badge'=? OR ("data_en"->>'has_video')::BOOLEAN=?) LIMIT 10`,
			expectedValues: []interface{}{"published", "video", true},
		},
		{
			...
			expectedSQL:    `SELECT data_en FROM repo WHERE status=? AND ("data_en"->>'badge'=? OR "data_en"->>'badge'=?) LIMIT 10`,
...
func TestQuery_AsSQL_InQuery(t *testing.T) {
	...
	tests := []testCase{
		...
		},
		{
			...
			expectedSQL:    `SELECT data_en FROM repo WHERE type IN (SELECT type FROM repo WHERE id=? LIMIT 10) LIMIT 10`,
			expectedValues: []interface{}{"002fd6b1-f715-4875-838b-1546f27327df"},
		},
		{
